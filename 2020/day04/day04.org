...
#+begin_src C++ :includes '(<vector> <numeric> <iostream> <map>) :namespaces std :flags -std=c++11 :results verbatim
  ...
  regex hairColorRegex(R"(#[0-9a-f]{6})");
  regex heightRegex(R"(([0-9]+)(cm|in))");
  regex passportIdRegex(R"([0-9]{9})");
  vector<string> validEyeColors = { "amb", "blu", "brn", "gry", "grn", "hzl", "oth" };

  bool foundInList(const vector<string>& list, const string& item) {
  ...
  int getValidFields (const string& line) {
      ...
      while (ss >> setw(3) >> key >> separator >> value) {
          if (key == "byr") {
              ...
              smatch match;
              if (regex_search(value, match, heightRegex)) {
                  int numericHeight = stoi(match.str(1));
                  string units = match.str(2);

                  if (units == "cm" && 150 <= numericHeight && numericHeight <= 193 ||
                      units == "in" && 59 <= numericHeight && numericHeight <= 76) {
                      ++validFields;
                  }
              }
          } else if (key == "hcl") {
              if (regex_match(value, hairColorRegex))
                  ++validFields;
          } else if (key == "ecl") {
              if (foundInList(validEyeColors, value))
                  ++validFields;
          } else if (key == "pid") {
  ...
  int main() {
      ...
      while (!file.eof()) {
          ...
          // Read one newline-separated record
          while(getline(file, line) && !line.empty()) {
              validFields += getValidFields(line);
          }
